...
//    @PostMapping("/add")
    ...
    public String addItemV4(@ModelAttribute("item") Item item, RedirectAttributes redirectAttributes){
        ...
        redirectAttributes.addAttribute("itemId",savedItem.getId());    // url 리다이렉트시 사용한다.
        redirectAttributes.addAttribute("status",true);    //  남은 status는 쿼리파라미터처럼 들어간다. ?status=true 꼴로 들어간다.
        return "redirect:/basic/items/{itemId}";  // addAttribute한 itemId가 여기로 치환되어 들어간다.
    }
